...
var xPositionScale = d3.scaleLinear().domain([1955, 2017]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 6000]).range([height, 0]);
var line = d3.line().x(function (d) {
  return xPositionScale(d.year);
}).y(function (d) {
  return yPositionScale(d.cumulativePoints);
}).curve(d3.curveStepAfter);
var colorScale = d3.scaleOrdinal().domain(['LeBron James', 'Michael Jordan', 'Kareem Abdul-Jabbar', 'Jerry West', 'Kevin Durant']).range(['#FDBB30', '#CE1141', '#333333', '#552582', '#006BB6']);
d3.tsv(require('./data/every-game.tsv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var nested = d3.nest().key(function (d) {
    return d.name;
  }).entries(datapoints); // Clean up each of the player groups

  nested.forEach(function (player) {
    var games = player.values; // Give them a .name instead of a .key

    player.name = player.key; // Is there name inside of the colorScale? If yes,
    // let's mark them as important

    player.important = colorScale.domain().indexOf(player.name) !== -1;
    player.firstYear = games[0].year; // Count the cumulative points as they go through
    // their career

    var cumulativePoints = 0;
    var cumulativeLastTwoRoundsPoints = 0;
    games.forEach(function (d, i) {
      d.index = i; // Get the year by itself

      d.careerYear = d.year - player.firstYear; // Keeping a running total of the points scored
  ...
  svg.selectAll('.player-line').data(nested).enter().append('path').attr('class', 'player-line').attr('d', function (d) {
    return line(d.values);
  }).attr('fill', 'none').classed('important', function (d) {
    return d.important;
  }).attr('stroke', '#CCCCCC').attr('stroke-width', 1); // Go back to the important people and
  // color/thicken their lines

  svg.selectAll('.important').attr('stroke', function (d) {
    return colorScale(d.name);
  }).attr('stroke-width', 2).raise(); // Now let's add labels for the important people

  var important = nested.filter(function (d) {
    return d.important;
  });
  svg.selectAll('.name-label').data(important).enter().append('text').attr('class', 'name-label').text(function (d) {
  ...
  }).attr('text-anchor', 'middle').attr('font-size', 12).attr('font-weight', 'bold').attr('dy', -5).style('text-shadow', '-1px -1px 0 #FFF, 1px -1px 0 #FFF, -1px 1px 0 #FFF, 1px 1px 0 #FFF');
  var yAxis = d3.axisLeft(yPositionScale).ticks(5).tickSize(3);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis);
  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.format('0')).ticks(5).tickSize(3);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  svg.selectAll('.axis line, .axis path');
}
},{"d3":"../node_modules/d3/index.js","./data/every-game.tsv":"data/every-game.tsv"}],"../../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","04-chart.js"], null)
//# sourceMappingURL=/04-chart.e580c6e2.map